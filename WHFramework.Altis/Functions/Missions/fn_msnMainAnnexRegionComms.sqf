...
// // Slightly raise structure above terrain
// {_x # 1 set [2, _x # 1 # 2 + 0.5]} forEach (_structure + _intel);

private _dir = [_pos, 0] call WHF_fnc_getDirSurfaceNormal;
_structure = [_structure, _pos, _dir, ["ASL", "path", "simple"], _objects] call WHF_fnc_objectsMapper;
_ground = [_ground, _pos, _dir, ["normal", "path", "simple"], _objects] call WHF_fnc_objectsMapper;
_tower = [_tower, _pos, _dir, ["frozen", "normal"], _objects] call WHF_fnc_objectsMapper;
_intel = [_intel, _pos, _dir, ["ASL", "frozen"], _objects] call WHF_fnc_objectsMapper;
_objects pushBack (_structure + _ground + _tower + _intel);

// If any intel is still buried in the ground, delete it to prevent soft locking
deleteVehicle (_intel select {getPosATL _x # 2 < -0.3});

private _building = _structure # 0;

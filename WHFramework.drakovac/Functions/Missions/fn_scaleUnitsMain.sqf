/*
Function: WHF_fnc_scaleUnitsMain

Description:
    Return a random integer in the given range, multiplied by
    the main mission units coefficient.
...
*/
params ["_min", "_max"];
if (isNil "_max") exitWith {ceil (_min * WHF_missions_main_units)};
private _num = _min + floor random (_max - _min + 1);
ceil (_num * WHF_missions_main_units)
